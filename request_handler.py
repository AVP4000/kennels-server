...
from views import get_customer_by_email, get_animals_by_location, get_employees_by_location, get_animals_by_status
...
class HandleRequests(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if '?' not in self.path:
            ...
            # Checks animals query for location_id key
            if query.get('location_id') and resource == 'animals':
                response = get_animals_by_location(query['location_id'][0])
            # Checks employees query for location id
            if query.get('location_id') and resource == 'employees':
                response = get_employees_by_location(query['location_id'][0])
            # Checks animals query for status
